...
class PermissionOfPhotos {
  Future<bool> getFromGallery(BuildContext context) async {
    ...
    if (await platform.status.isDenied) {
      ...
      //   await platform.request();
      // } else {
      //   Get.log("in first else");
      //   // await Permission.storage.request();
      //   await platform.request();

        // showDeleteDialog(context,
        //     "TAG app requires access to the gallery to upload pictures");
      // }
      ...
      if (Platform.isIOS) {
        showDialog(
            ...
            builder: (BuildContext context) => CupertinoAlertDialog(
                  title: Text("Settings"),
                  content: Text(
                      "TAG App want to access camera open settings and give permission"),
                  actions: <Widget>[
  ...
  Future<bool> getFromCamera(BuildContext context) async {
    ...
    if (await Permission.camera.status.isDenied) {
      if (Platform.isIOS) {
        ...
        // showDeleteDialog(context,
        //     "TAG app requires access to the camera to upload pictures");
    ...
    if (await Permission.camera.status.isPermanentlyDenied) {
      if (Platform.isIOS) {
        ...
        showDeleteDialog(context,
            "TAG app requires access to the camera to capture pictures");
      }
